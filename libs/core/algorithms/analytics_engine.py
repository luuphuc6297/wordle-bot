...
class AnalyticsEngine:
    ...
    for first_word in high_entropy_words[:10]:
      for second_word in high_entropy_words:
        if first_word != second_word:
          avg_entropy = self._calculate_combination_entropy(first_word, second_word)
    ...
    for word in sample_words:
      ...
      for _i, guess_info in enumerate(game_result["guesses"]):
        ...
        if remaining_before > 0:
          reduction_ratio = (remaining_before - remaining_after) / remaining_before
  ...
  def generate_strategy_insights(self) -> dict[str, Any]:
    ...
    insights = {
      "position_insights": {
        ...
        "entropy_by_position": [p.entropy_contribution for p in position_analysis],
      },
      "pattern_insights": {
        "most_effective_pattern": feedback_analysis["most_effective_patterns"][0][0],
  ...
  def _calculate_combination_entropy(self, first_word: str, second_word: str) -> float:
