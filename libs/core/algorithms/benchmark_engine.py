...
class BenchmarkEngine:
  ...
  def run_benchmark(
    ...
    for i, target_word in enumerate(target_words):
      try:
        ...
        if show_progress:
          avg_guesses = (
            statistics.mean(total_guesses_for_wins) if total_guesses_for_wins else 0
          )
          self.display.update_progress(i + 1, wins, avg_guesses)

      except Exception as e:
        self.logger.error(f"Error in game {i + 1} with target '{target_word}': {e}")
        continue
  ...
  def _play_single_game(self, target_word: str) -> dict[str, Any]:
    ...
    while not game_manager.is_game_over() and turn <= 6:
      ...
      if len(current_answers) > 1:
        entropy_calc = solver.calculate_detailed_entropy(best_guess, current_answers)
  ...
  def _compile_results(
    ...
    median_guesses = statistics.median(guess_counts_wins) if guess_counts_wins else 0
    ...
    return {
      ...
      "hardest_words": [(r["target_word"], r["guesses_used"]) for r in hardest_wins],
      "easiest_words": [(r["target_word"], r["guesses_used"]) for r in easiest_wins],
  ...
  def run_stress_test(self, difficult_words: list[str] | None = None) -> dict[str, Any]:
    ...
    if difficult_words is None:
      ...
      difficult_words = [w for w in difficult_words if self.lexicon.is_valid_answer(w)]
  ...
  def analyze_algorithm_performance(self, results: dict[str, Any]) -> dict[str, Any]:
    ...
    if results["avg_guesses"] > 4.0:
      recommendations.append("Algorithm may benefit from improved entropy calculation")
