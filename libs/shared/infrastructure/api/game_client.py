...
class GameClient:
  ...
  def submit_guess(self, guess: str) -> GuessResult:
    ...
    try:
      ...
      if "result" not in data:
        raise WordleAPIError("Invalid API response format: missing 'result' field")
      ...
      raise WordleAPIError(f"Invalid API response for guess '{guess}': {str(e)}") from e
  ...
  def _validate_response(self, response: requests.Response) -> None:
    ...
    if not content_type.startswith("application/json"):
      raise WordleAPIError(f"Expected JSON response, got content-type: {content_type}")
