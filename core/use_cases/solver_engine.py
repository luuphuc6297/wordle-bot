...
from core.domain.models import FeedbackType, EntropyCalculation
...
class SolverEngine:
    ...
    def find_best_guess(self, possible_answers: List[str], turn: int = 1) -> str:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for guess_word in self._all_guesses:
                if time.time() - start_time > self.time_budget * 0.9:  # Leave some buffer
                    break
                
                future = executor.submit(
                    self._calculate_entropy_for_word, 
                    guess_word, 
                    possible_answers_array
                )
    ...
    def _calculate_entropy_for_word(self, guess_word: str, possible_answers: np.ndarray) -> float:
    ...
    def calculate_detailed_entropy(self, guess_word: str, possible_answers: List[str]) -> EntropyCalculation:
