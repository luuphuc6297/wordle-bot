...
class WordLexicon:
    ...
    def _load_word_lists(self):
        ...
        with open(answers_path, 'r', encoding='utf-8') as f:
            self._answers = [line.strip().upper() for line in f if line.strip() and len(line.strip()) == 5]
        ...
        with open(allowed_path, 'r', encoding='utf-8') as f:
            self._allowed_guesses = [line.strip().upper() for line in f if line.strip() and len(line.strip()) == 5]
        ...
        if len(self._allowed_guesses) < len(self._answers):
            raise ValueError(f"Allowed guesses ({len(self._allowed_guesses)}) should be >= answers ({len(self._answers)})")
