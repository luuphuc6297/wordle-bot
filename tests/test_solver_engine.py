...
class TestFeedbackSimulation:
    ...
    def test_duplicate_letters_in_guess_single_in_answer(self, solver):
        ...
        assert solver._simulate_feedback("SPEED", "CRANE") == "-----"  # No S,P,E,E,D in CRANE
        assert solver._simulate_feedback("GEESE", "CRANE") == "--o--"  # Only one E should be yellow
        assert solver._simulate_feedback("ERASE", "CRANE") == "o-+--"  # E yellow, A green
        
        # More complex case
        assert solver._simulate_feedback("ALLEY", "PLANE") == "+--o-"  # A green, L yellow (only one)
    ...
    def test_duplicate_letters_in_answer_single_in_guess(self, solver):
        ...
        assert solver._simulate_feedback("PLANE", "ALLEY") == "-+---"  # L green at position 2
        assert solver._simulate_feedback("LOGIC", "ALLEY") == "+----"  # L green at position 1
        assert solver._simulate_feedback("CHILD", "ALLEY") == "----o"  # L yellow (not in position)
    
    def test_duplicate_letters_both_guess_and_answer(self, solver):
        """Test duplicates in both guess and answer."""
        # Both have double L
        assert solver._simulate_feedback("ALLEY", "LLAMA") == "-++--"  # Both Ls match positions
        
        # Different duplicate patterns
        assert solver._simulate_feedback("SPEED", "ERASE") == "o-oo-"  # Complex E handling
        assert solver._simulate_feedback("GEESE", "ERASE") == "---o+"  # E,S,E all handled correctly
    ...
    def test_edge_cases(self, solver):
        ...
        # Anagram
        assert solver._simulate_feedback("LISTEN", "SILENT") == "ooooo-"  # All letters present but wrong positions
    ...
    def test_known_wordle_examples(self, solver):
        ...
        for guess, answer, expected in test_cases:
            ...
            assert result == expected, f"Failed: {guess} vs {answer}, got {result}, expected {expected}"
