...
from core.domain.models import GuessResult, FeedbackType
...
class GameDisplay:
    ...
    def show_guess_submission(self, turn: int, guess: str, 
        ...
        if self.show_detailed and entropy is not None:
            print(f"[DEBUG] {timestamp} - Entropy: {entropy:.3f} bits | "
                  f"Calculation time: {calculation_time:.3f}s")
        
        if remaining_count is not None:
            print(f"[DEBUG] {timestamp} - Searching among {remaining_count:,} possible answers")
    ...
    def show_feedback(self, guess_result: GuessResult, remaining_count: int):
        ...
        print(f"[INFO] {timestamp} - Pattern: {guess_result.guess} -> {''.join(symbol_feedback)}")
    ...
    def show_word_analysis(self, word: str, entropy: float, patterns: int, 
                          remaining: int):
        ...
        print(f"  📈 {word}: {entropy:.3f} bits | {patterns} patterns | "
...
class BenchmarkDisplay:
    ...
    def update_progress(self, completed: int, wins: int, avg_guesses: float):
        ...
        print(f"\r🔄 Progress: {completed}/{self.total_games} ({progress_percent:.1f}%) | "
              f"Win Rate: {win_rate:.1f}% | Avg Guesses: {avg_guesses:.2f}", end="")
    ...
    def show_final_results(self, results: Dict[str, Any]):
        ...
        for guess_count, count in results['distribution'].items():
            if guess_count == 'losses':
                ...
                print(f"  {guess_count} Guess{'es' if guess_count != 1 else ''}: "
